...
function CalendarContainer({ isExpanded, minDate, maxDate, startDate, endDate, handleDateChange, handleClickSearch, handleHideCalendar }) {
  ...
  return (
      <div className={`calendar-container ${isExpanded ? '' : 'hide'}`} >
          ...
          <div className='button__container calendar-container__button-container' >
              <button className="button--transparent" onClick={handleHideCalendar} >Cancel</button>
              <button className="button--blue" onClick={handleClickSearch } >SEARCH</button>
          </div>
      </div>
  );
}
